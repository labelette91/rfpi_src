		...
		while ( ! myEventManager->eventList.empty() ) {
			...
			case RFRPI_EVENT_INIT:
				...
				if ( Singleton::get() ) {
					...
					printf("[INFO] Welcome message is send --- PING ! \n");
			...
			case RFRPI_EVENT_GETSENSORDATA:
...

				{
					...
					if ( s != NULL && s->isDecoded() ) {
						...
						printf(" {\"datetime\": \"%s\", \"temperature\": \"%f\", \"humidity\": \"%f\", \"channel\": \"%d\" } \n",
						...
						if ( s == NULL ) {
							...
							if ( ev.strValue != NULL && strlen(ev.strValue) >= 11 ) {
								if (strcmp(ev.strValue,"RCSW 123456") == 0) {
									printf("[INFO] Welcome message correctly received, Remote TX works fine Local / Rx works fine \n");
									printf("[INFO] Welcome message is send back --- PONG ! \n");
									Singleton::get()->getCore433()->sendMess(0x654321,CORE433_MOD_RCS);
								}
							}
							if ( ev.strValue != NULL && strlen(ev.strValue) >= 11 ) {
                            	if (strcmp(ev.strValue,"RCSW 654321") == 0) {
                            		printf("[INFO] Welcome message correctly received, Local TX/RX works fine --- PONG\n");
                            	}
                            }
						}
					}
